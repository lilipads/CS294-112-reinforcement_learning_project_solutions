...
class Exemplar(Density_Model):
    ...
    def build_computation_graph(self):
        ...
        self.kl = tf.distributions.kl_divergence(self.encoder1, self.prior) + tf.distributions.kl_divergence(
            self.encoder2, self.prior)
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.Variable(tf.zeros(z_size), name='sy_logstd')
    ...
    def make_discriminator(self, z, output_size, scope, n_layers, hid_size):
        """
            ...
            args:
                z: input to discriminator network
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.likelihood, feed_dict={self.state1: state1,
            self.state2: state2, self.discrim_target: np.ones((state1.shape[0], 1))})
        return likelihood
